...
// newResourceClaimValidator returns a ResourceClaimValidator with the DRA driver
// enabled and the given VF mode and operator namespace.
func newResourceClaimValidator(vfMode bool) *validator.ResourceClaimValidator {
	cph := validator.NewClusterPolicyHandler()
	cph.SetDraDriverEnabled(true)
...
var _ = Describe("ResourceClaim", func() {
	...
	Context("DRA driver disabled", func() {
		// With draDriver false the classic device plugin allocates Spyre, so the
		// ResourceClaim webhook imposes no restriction regardless of VF mode,
		// namespace, or DeviceClass.
		newDisabled := func(vfMode bool) *validator.ResourceClaimValidator {
			cph := validator.NewClusterPolicyHandler()
			cph.SetDraDriverEnabled(false)
			cph.SetVFModeEnabled(vfMode)
			cph.SetOperatorNamespace(operatorNamespace)
			return validator.NewResourceClaimValidator(cph)
		}

		It("allows every Spyre DeviceClass in any namespace", func() {
			deviceClasses := []string{
				validator.DeviceClassSpyrePF,
				validator.DeviceClassSpyrePrivilegedVF,
				validator.DeviceClassSpyreStandardVF,
			}
			for _, vfMode := range []bool{true, false} {
				v := newDisabled(vfMode)
				for _, dc := range deviceClasses {
					Expect(v.Validate(generalNamespace, buildClaim(dc))).To(BeNil())
				}
			}
		})
	})
