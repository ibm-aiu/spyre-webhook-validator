...
//
// The rules only apply when the DRA driver is enabled
// (SpyreClusterPolicy.spec.devicePlugin.draDriver == true). When it is disabled,
// Spyre is allocated through the classic device plugin and ResourceClaims are
// not the Spyre allocation mechanism, so no DeviceClass restriction is imposed.
func (v *ResourceClaimValidator) Validate(namespace string, raw []byte) error {
	if !v.clusterPolicyHandler.draDriverEnabled {
		return nil
	}
	...
	vfMode := v.clusterPolicyHandler.vfModeEnabled
	operatorNS := v.clusterPolicyHandler.operatorNamespace
